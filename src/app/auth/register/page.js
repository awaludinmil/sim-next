...
export default function RegisterPage() {
  ...
  const handleNumberClick = (num) => {
    if (pin.length < maxPinLength) {
      ...
      // Jika PIN sudah 6 digit, simpan dan lanjut ke OTP
      if (newPin.length === maxPinLength) {
        setTimeout(() => {
          saveUserAndProceed(newPin);
        }, 200);
      }
    }
  };
  ...
  const saveUserAndProceed = (userPin) => {
    // Simpan user baru dengan PIN
